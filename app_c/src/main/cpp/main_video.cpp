...
void *thread_capture(void *argu)
{
    ...
    struct MSDKEncoder *gpuEncoder = CreateEncoder(&InputParam);
	if (gpuEncoder == NULL)
	{
    ...
    while (total_frame_count < 50)
    {
		...
		int status = EncodeFrame(gpuEncoder,&_sSrcPic, &_sLayerData);
		if (status == MCODEC_SUCCEED)
		{
			status = GetBitstream(gpuEncoder,&_sLayerData);
			if (status == MCODEC_SUCCEED)
			{
